...
function displayData(data) {
    ...
    if (data.error) {
        ...
        let currentHTML = `<h3>${data.timezone}</h3>
							<p>${today.toLocaleString("en-us", {
                                weekday: "long",
                            })},${today.toLocaleString("en-us", { month: "long" })}, ${today.getDate()}</p>
        ...
        for (let i = 0; i < 8; i++) {
            ...
            forecastHTML += `<section class="day">
								<h3><span>${date.toLocaleString("en-us", { weekday: "long" })}</span> ${date.toLocaleString("en-us", {
                month: "long",
            })} ${date.getDate()}</h3>
...
async function getWeather(location) {
    ...
    /*
		...
		*/
    let endpoint = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=temperature_2m_max,temperature_2m_min,weather_code&forecast_days=8&current=temperature_2m,weather_code&timezone=auto&temperature_unit=fahrenheit`;
